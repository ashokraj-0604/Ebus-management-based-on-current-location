...
function searchBuses(source, destination) {
    ...
    console.log(`Searching buses in Firestore for source: ${source} and destination: ${destination}`);
    ...
    db.collection('buses')
        ...
        .then((querySnapshot) => {
            if (querySnapshot.empty) {
                ...
                console.log('No buses found for the specified route');
                return;
            }

            // Clear loading text
            busListContainer.innerHTML = '';
            console.log(`Found ${querySnapshot.size} buses for the route`);
